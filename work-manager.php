...
if(isset($_GET['delete'])):
   $id = $_GET['delete'];
    $result = mysql_query("DELETE from `workdata` where `workdata_id`='$id'") or die(mysql_error());
    header("location:work-manager.php");
endif;

if(isset($_POST['edit'])):
  if(editWorkData($_POST)):
   header("location:work-manager.php");
  endif;
endif;
...
if(isset($_POST['workdata'])):
	$workans = saveWorkData($_POST);
    if($workans == false){
	?>
	<script>
	alert("Sorry Unable to add new workdata requirement because it already exist");
	window.location = "work-manager.php";
	</script>
	<?php
    }else{
	?>
	<script>
	alert("You successfully added new wordata record");
	window.location = "work-manager.php";
	</script>
	<?php
	}
...
<li>
                        <a href="#" class="style1">Work Details</a>                    </li>
              </ul>
                 ...
                 <section class="widget">
                    <header>
                        <h4>
                            ...
                            Work Details List                        </h4> <div class="pull-right"><button class="btn btn-danger btn-large" id="add"><i class=" icon-building" > </i> &nbsp;Add Work Details</button></div>
						   <blockquote>
                            Column sorting, live search, pagination.                        </blockquote>
                   </header>
                    <div class="body">
                        ...
                        <table id="datatable-table" class="table table-striped">
						 ...
						 extract($row); 
						  ...
						  ?>
						  <tr><td><?=$y; ?></td><td><?=$label; ?></td>
						  <td><div class="btn-group">
                                  <a class="btn btn-primary" href="#"><i class="icon-building icon-white"></i></a>
                                  <a class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#"><span class="caret"></span></a>
                                  <ul class="dropdown-menu">
                                    <li><a href="#" class="edit" id="<?=$workdata_id; ?>" name="<?=$label; ?>"><i class="icon-edit"></i> Edit </a></li>
                                    <li><a href="#" data-href="work-manager.php?delete=<?=$workdata_id; ?>" class="delete" id="<?=$workdata_id; ?>" name="<?=$label; ?>"><i class="icon-remove"></i> Delete</a></li>
...
<!-- modals -->
	<div id="modal-edit" class="modal hide fade" tabindex="-1" role="dialog">
						<form action="work-manager.php" method="post" id="form-final" enctype="multipart/form-data" >
                    ...
                    <div id="modal-requirement" class="modal hide fade" tabindex="-1" role="dialog">
                        <form action="work-manager.php" method="post" id="form-final" enctype="multipart/form-data" >
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal">&times;</button>
                                <h4 id="myModalLabel2">Add Work Data</h4>
                            </div>
                            <div class="modal-body" id="requirement-edit-body">
                               <table class="table table-bordered table-condensed table-hover table-striped">
                                <tbody>
                                <tr><td width="60%">Name:</td><td><input type="text" name="label" id="label" required /></td></tr>
                            ...
                            <div class="modal-footer">
                                <button class="btn" data-dismiss="modal">Close</button>
                               <button class="btn btn-primary" type="submit" name="workdata">Save changes</button>
                            </div>
                        </form>
                        </div>
					...
					<div id="modal-account" class="modal hide fade" tabindex="-1" role="dialog">
                        ...
                        <div id="modal-delete" class="modal hide fade" tabindex="-1" role="dialog">
                            ...
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal">&times;</button>
                                <h4 id="myModalLabel2">Delete Work Data</h4>
                            </div>
                            <div class="modal-body" id="work-edit-body">
                               <h3>Are you sure you want to delete <span id="rname"></span>?<br>
                               All associated data will also be deleted.</h3>
                            </div>
                            <div class="modal-footer">
                                <button class="btn" data-dismiss="modal">Close</button>
...
var href="";
    ...
    href=$(this).attr("data-href");
        var workdata_id = $(this).attr("id");
         var name=$(this).attr('name');
         $("#rname").text(name);
        $.post('ajax/ajax.php?getWorkData', "workdata_id="+workdata_id, function(data) {
            $("#tbody").html(data);
