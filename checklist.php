    ...
    <div class="content container-fluid">
        <div class="row-fluid">
			...
			<thead >
                ...
                <?php
                $iarray=[];
                    $initial = mysql_query("select * from `checklist` where `department_id` = '$department_id' and `type`=1") or die(mysql_error());
                    if(mysql_num_rows($initial)):
                        while($row=mysql_fetch_array($initial)):
                            $iarray[]=$row['checklist_id'];
                        ?>
                        <th style="font-size:9px;"><?=$row['name']; ?></th>
                    <?php
                        endwhile;
                    endif;
                ?>
				<?php
                $farray = [];
                    $final = mysql_query("select * from `checklist` where `department_id` = '$department_id' and `type`=0") or die(mysql_error());
                    if(mysql_num_rows($final)):
			   ...
			   ?>
			   <tr style="border:1px solid #618fb0; font-size:9px;"><td><?=$x; ?></td><td><?=$name; ?></td>
                <?php
                foreach($iarray as $checklist_id):
                    ?>
                    <td align="center"><?=check($student_id,$checklist_id); ?></td>
                    <?php
                endforeach;
                ?>
                 <?php
                foreach($farray as $checklist_id):
                    ?>
                    <td align="center"><?=check($student_id,$checklist_id); ?></td>
                    <?php
                endforeach;
